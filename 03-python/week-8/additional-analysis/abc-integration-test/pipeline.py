...
def run_pipeline():
 ...
 # TRANSFORM: Roll B ühendab ja töötleb Roll A-st saadud andmed
        ...
        logger.info(f"Merged DataFrame shape: {df.shape}")
                                                                                                                        ...
                                                                                                                        # AJUTINE KONTROLL: vaatame võtmeväljade esimesi ridu
        print(df[['id', 'sale_id', 'customer_id', 'sale_date', 'total_price']].head(10).to_string(index=False))
        print("id unique:", df['id'].nunique(), "rows:", len(df))
        print("sale_id unique:", df['sale_id'].nunique(), "rows:", len(df))

        print("Duplicate invoice_id:", df.duplicated(subset=['invoice_id']).sum())
        print("NULL customer_id:", df['customer_id'].isna().sum())
        print("NULL sale_date:", df['sale_date'].isna().sum())
        print("NULL total_price:", df['total_price'].isna().sum())
        print("total_price <= 0:", (df['total_price'] <= 0).sum())

        print("NULL customer_id AND total_price <= 0:",
        (df['customer_id'].isna() & (df['total_price'] <= 0)).sum())

        print("Rows invalid by cleaning rules:",
        (df['customer_id'].isna() | (df['total_price'] <= 0)).sum())
                                                                                                                         # AJUTINE KONTROLLI LÕPP
